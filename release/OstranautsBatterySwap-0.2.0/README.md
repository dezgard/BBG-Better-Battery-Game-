...
Version 0.2.0 adds charger support. The mod uses carried spare batteries first, then checks compatible ship chargers if no carried spare is available.
...
```text
Ostranauts Battery Swap 0.2.0 loaded.
```
...
```text
release\OstranautsBatterySwap-0.2.0\
```
